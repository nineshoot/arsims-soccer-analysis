...
|**`predictions_log.csv` written *before* kickoff**|The one file that makes the whole thing trustworthy: no hindsight, real calibration. Only matches dated today or later are predicted, and a settled row is final — no later prediction may replace it. Early runs broke both rules (fixtures.csv lists matches for a while after they're played); those rows carry `post_match = True` and are left out of every hit rate and calibration curve rather than deleted.|
