...
|**`predictions_log.csv` written *before* kickoff**|The one file that makes the whole thing trustworthy: no hindsight, real calibration. Each row records its `kickoff_utc`; only matches that haven't kicked off are predicted, and a settled row is final — no later prediction may replace it. Early runs broke both rules (fixtures.csv lists matches for a while after they're played); any row logged at or after kickoff carries `post_match = True` and is left out of every hit rate and calibration curve rather than deleted. Judged by kickoff, not date — a Friday 19:00 game predicted at 22:36 the same evening is post-match.|
